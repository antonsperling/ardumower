...
Mower::Mower(){
  ...
  motorAccel                 = 1000;      // motor wheel acceleration - only functional when odometry is not in use (warning: do not set too low)
    ...
    motorZeroSettleTime        = 3000 ;     // how long (ms) to wait for motors to settle at zero speed
		motorReverseTime           = 2400;      // max. reverse time (ms)
  ...
  // ------ lawn sensor --------------------------------
  lawnSensorUse     = 1;                   // use capacitive lawn Sensor
...
void Mower::setup(){
 ...
 // drops
  ...
  if (esp8266Use) {
    ...
    ESP8266port.flush();
    ESP8266port.end();
    // Remote Control only for Bluetooth. But want to use ESP8266 as client for gps data
