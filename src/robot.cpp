...
Robot::Robot(){
  ...
  lawnSensorCounter = 0;
  lawnSensor = false;
  lawnSensorLeft = lawnSensorRight = false;
...
//NOTE: this function should only read in sensors into variables - it should NOT change any state!
  ...
  if ((lawnSensorUse) && (millis() >= nextTimeLawnSensor)){    
    ...
    Console.print("LawnSensors f/b = ");
    Console.print(lawnSensorFront);
    Console.print("/");
    Console.println(lawnSensorBack);     
  }
 if ((lawnSensorUse) && (millis() >= nextTimeLawnSensorCheck)){          
    ...
    if ((deltaFront <= 95) || (deltaBack <= 95)){
      ...
      if (deltaFront <= 95) {
        ...
        lawnSensorRight = true;
        setSensorTriggered(SEN_LAWN_BACK);
      }
      lawnSensor=true;
    }
...
void Robot::checkLawn(){
  ...
  if ( (lawnSensor) && (millis() > stateStartTime + 3000) ) {      
    ...
    lawnSensor = false;
    lawnSensorLeft = false;
    lawnSensorRight = false;
  }
}
